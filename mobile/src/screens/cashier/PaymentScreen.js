...
const PaymentScreen = ({ route, navigation }) => {
  ...
  const systemVerificationType =
    discountScope || customerVerification?.type || null;
  const systemVerificationSource =
    customerVerification?.verificationSource || "system";
  ...
  const systemRemainingPurchaseCap =
    discountSnapshot.remainingPurchaseCap || PURCHASE_CAP;
  const systemRemainingDiscountCap =
    discountSnapshot.remainingDiscountCap || WEEKLY_CAP;
  ...
  const systemWeekEndUsage =
    weeklyUsageSnapshot.weekEnd || currentUsage?.weekEnd;
  ...
  const fivePercentDiscount = useMemo(() => {
    ...
    const isEligibleForDiscount =
      (manualVerification.verified || systemVerified) &&
      (currentVerificationType === "senior" ||
        currentVerificationType === "pwd");
  ...
  const handleManualVerify = (type) => {
    Alert.alert(
      ...
      appUser
        ? "Check physical ID presented by customer"
  ...
  const handleFinalize = async () => {
    try {
      ...
      if (
        (finalVerificationType === "senior" ||
          finalVerificationType === "pwd") &&
        !bookletUpdated
      ) {
        Alert.alert(
          "Booklet Not Updated",
  ...
  const proceedWithPayment = async () => {
    try {
      ...
      if (
        ...
        if (bookletUsed + fivePercentDiscount > WEEKLY_CAP) {
          Alert.alert(
            ...
            "Total discount exceeds weekly cap of ₱125",
          );
          return;
        }
        ...
        if (bnpcEligibleSubtotal > PURCHASE_CAP) {
          Alert.alert(
            ...
            "BNPC purchase exceeds weekly limit of ₱2,500",
          );
          return;
        }
      }
      ...
      const verificationSource = manualVerification.override
        ? "manual"
        : systemVerified
      ...
      >
        ...
        {systemVerified && !manualVerification.override && !appUser && (
          <View style={styles.systemVerifiedBadge}>
            ...
            <Text style={styles.systemVerifiedText}>
              System verified as{" "}
              {systemVerificationType === "senior"
                ? "Senior"
                : systemVerificationType === "pwd"
                  ? "PWD"
                  : "Regular"}
            </Text>
          </View>
          ...
          !manualVerification.override && (
            <View style={styles.autoVerifiedBadge}>
              ...
              <Text style={styles.autoVerifiedText}>
                Auto-verified from app:{" "}
                {manualVerification.type === "senior"
                  ? "Senior"
                  : manualVerification.type === "pwd"
                    ? "PWD"
                    : "Regular"}
              </Text>
            </View>
        ...
        <View style={styles.section}>
          ...
          {showVerificationOptions ? (
            ...
            <View
              style={[
                styles.verifiedBox,
                manualVerification.type === "regular" &&
                  styles.regularVerifiedBox,
              ]}
            >
                ...
                >
                  {manualVerification.type === "senior"
                    ...
                    : manualVerification.type === "pwd"
                      ...
                      : "Regular Customer"}{" "}
                  Verified
                  {!manualVerification.override && appUser
                    ? " (Auto)"
        ...
        {isCustomerVerifiedForDiscount && (
          <View style={styles.section}>
            ...
            <View style={styles.capsDisplay}>
              <View style={styles.capItem}>
                ...
                <Text style={styles.capValue}>
                  ₱{remainingDiscountCap.toFixed(2)}
                </Text>
              </View>
              ...
              <View style={styles.capItem}>
                ...
                <Text style={styles.capValue}>
                  ₱{remainingPurchaseCap.toFixed(2)}
                </Text>
              </View>
            </View>
            ...
            {fivePercentDiscount > 0 && (
              <View style={styles.discountAppliedBox}>
                <View style={styles.discountAppliedHeader}>
                  <Ionicons name="checkmark-circle" size={16} color="#10B981" />
                  <Text style={styles.discountAppliedTitle}>
                    5% Discount Applied
                  </Text>
                </View>
            ...
            {serverBnpcDiscount > 0 &&
              fivePercentDiscount !== serverBnpcDiscount && (
            ...
            {bnpcEligibleSubtotal > 0 && (
              <View style={styles.amountRow}>
                ...
                <Text style={styles.amountValue}>
                  ₱{bnpcEligibleSubtotal.toFixed(2)}
                </Text>
              </View>
            )}
