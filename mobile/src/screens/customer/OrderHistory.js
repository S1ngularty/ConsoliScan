...
// FIX: scale (native) and backgroundColor (JS) must live on SEPARATE Animated.Views
const FilterChip = ({ label, isActive, onPress }) => {
  const scale = useRef(new Animated.Value(1)).current;   // useNativeDriver: true
  const bg    = useRef(new Animated.Value(isActive ? 1 : 0)).current; // useNativeDriver: false
  ...
  useEffect(() => {
    Animated.timing(bg, {
      ...
      useNativeDriver: false, // color interpolation must be JS-driven
    }).start();
  }, [isActive]);
  ...
  const handlePress = () => {
    Animated.sequence([
      ...
      Animated.timing(scale, { toValue: 1,    duration: 150, useNativeDriver: true, easing: Easing.out(Easing.back(1.5)) }),
    ]).start();
    onPress();
  };
  ...
  return (
    // Outer: JS-driven (backgroundColor, borderColor)
    <Animated.View style={[styles.filterChip, { backgroundColor, borderColor }]}>
      {/* Inner: native-driven (scale only) */}
      <Animated.View style={{ transform: [{ scale }] }}>
        <TouchableOpacity onPress={handlePress} activeOpacity={1} style={styles.filterChipInner}>
...
// ─── Animated Order Card ─────────────────────────────────────────────────────
// FIX: mount/scale (native) are fine together. Download spin is native too.
// Removed mixing — no JS-driver values on the same node as native-driver values.
const OrderCard = ({ order, onDetails, onDownload, downloadingId, index }) => {
  ...
  // Staggered mount fade + slide (both native)
  useEffect(() => {
    Animated.timing(mountAnim, {
      toValue:         1,
      duration:        300,
      delay:           Math.min(index * 55, 300), // cap stagger so long lists don't lag
      easing:          Easing.out(Easing.cubic),
      useNativeDriver: true,
    }).start();
  }, []);

  // Spin icon while downloading (native)
  useEffect(() => {
    if (isDownloading) {
      spinAnim.setValue(0);
  ...
  return (
    // mount: native opacity + translateY; press: native scale — all on one node, all native ✓
    <Animated.View style={[styles.orderCard, { opacity, transform: [{ translateY }, { scale }] }]}>
      <TouchableOpacity
        activeOpacity={1}
        ...
        <View style={styles.orderFooter}>
          {totalDiscount > 0 ? (
            <View style={styles.chip}>
              ...
              <Text style={[styles.chipText, styles.discountChipText]}>Discount: {formatPrice(totalDiscount)}</Text>
            </View>
          ) : (
            <View style={styles.chip}>
      ...
      {/* Download — outside card tap area */}
      <TouchableOpacity
      ...
      >
        {isDownloading ? (
          <>
            {/* spin is native-driven on its own Animated.View ✓ */}
            <Animated.View style={{ transform: [{ rotate }] }}>
              <MaterialCommunityIcons name="loading" size={18} color="#00A86B" />
            </Animated.View>
...
// ─── Order Details Modal ─────────────────────────────────────────────────────
// FIX: backdrop opacity (native) and sheet translateY (native) — both native, no mixing ✓
const OrderDetailsModal = ({ order, visible, onClose, onDownload, downloadingId }) => {
  const slideY   = useRef(new Animated.Value(700)).current; // useNativeDriver: true
  const backdrop = useRef(new Animated.Value(0)).current;   // useNativeDriver: true
    ...
    if (visible) {
      Animated.parallel([
        ...
        Animated.spring(slideY, {
          toValue: 0, bounciness: 3, speed: 16, useNativeDriver: true,
        }),
      ]).start();
    }
  }, [visible]);
  ...
  const handleClose = useCallback(() => {
    Animated.parallel([
      ...
      Animated.timing(slideY, {
        toValue: 700, duration: 240, easing: Easing.in(Easing.quad), useNativeDriver: true,
      }),
    ]).start(() => onClose());
  }, [onClose]);
  ...
  return (
    <View style={styles.modalOverlay} pointerEvents="box-none">
      {/* Backdrop — native opacity only ✓ */}
      <Animated.View style={[styles.backdrop, { opacity: backdrop }]} pointerEvents="auto">
        <TouchableOpacity style={{ flex: 1 }} activeOpacity={1} onPress={handleClose} />
      </Animated.View>

      {/* Sheet — native translateY only ✓ */}
      <Animated.View style={[styles.modalContainer, { transform: [{ translateY: slideY }] }]}>
            ...
            <View style={styles.infoCard}>
              ...
              <View style={styles.infoRow}>
                ...
                <View style={styles.infoText}>
                  ...
                  <Text style={[styles.infoValue, styles.statusValue]}>{order.status || "Confirmed"}</Text>
                </View>
              </View>
            </View>
          </View>
          ...
          <View style={styles.modalSection}>
            ...
            <View style={styles.modalItemsList}>
              {(order.items || []).map((item, i) => (
                <View key={i} style={styles.modalItemCard}>
                  ...
                  <View style={styles.modalItemInfo}>
                    ...
                    <View style={styles.modalItemDetails}>
                      ...
                      <Text style={styles.modalItemUnitPrice}>₱{(item.unitPrice || 0).toFixed(2)} each</Text>
                    </View>
                  </View>
            ...
            <View style={styles.pricingCard}>
              ...
              {order.bnpcEligibleSubtotal > 0 && (
                <View style={styles.pricingRow}>
                  ...
                  <Text style={styles.pricingValue}>{formatPrice(order.bnpcEligibleSubtotal)}</Text>
                </View>
              )}
              {bnpcDiscount > 0 && (
                <View style={styles.pricingRow}>
                  <View style={styles.discountRow}>
                    ...
                    <Text style={[styles.pricingLabel, styles.discountLabel]}>BNPC Discount (5%)</Text>
                  </View>
                  <Text style={[styles.pricingValue, styles.discountValue]}>-{formatPrice(bnpcDiscount)}</Text>
                </View>
              )}
              {promoDiscount > 0 && (
                <View style={styles.pricingRow}>
                  <View style={styles.discountRow}>
                    <MaterialCommunityIcons name="sale" size={14} color="#FF9800" />
                    <Text style={[styles.pricingLabel, styles.promoLabel]}>Promo {order.promoDiscount?.code || ""}</Text>
                  </View>
                  <Text style={[styles.pricingValue, styles.promoValue]}>-{formatPrice(promoDiscount)}</Text>
                </View>
              )}
              {loyaltyDiscount > 0 && (
                <View style={styles.pricingRow}>
                  <View style={styles.discountRow}>
                    ...
                    <Text style={[styles.pricingLabel, styles.loyaltyLabel]}>Loyalty Points</Text>
                  </View>
                  <Text style={[styles.pricingValue, styles.loyaltyValue]}>-{formatPrice(loyaltyDiscount)}</Text>
                </View>
              )}
              {totalDiscount > 0 && (
                <View style={styles.totalDiscountRow}>
                  <Text style={styles.totalDiscountLabel}>Total Discount</Text>
                  <Text style={styles.totalDiscountValue}>-{formatPrice(totalDiscount)}</Text>
                </View>
              )}
            ...
            <View style={styles.modalSection}>
              ...
              <View style={styles.capsCard}>
                <View style={styles.capRow}>
                  ...
                  <Text style={styles.capValue}>₱{order.bnpcCaps.discountCap.usedAfter.toFixed(2)} / 125</Text>
                </View>
                <View style={styles.capRow}>
                  <Text style={styles.capLabel}>Purchase Used This Week</Text>
                  <Text style={styles.capValue}>₱{order.bnpcCaps.purchaseCap.usedAfter.toFixed(2)} / 2500</Text>
                </View>
                {order.bnpcCaps.weekEnd && (
                  <Text style={styles.capNote}>Resets: {new Date(order.bnpcCaps.weekEnd).toLocaleDateString()}</Text>
                )}
              </View>
            </View>
          )}
              ...
              <View style={styles.pointsCard}>
                {pointsUsed > 0 && (
                  <View style={styles.pointsRow}>
                    <MaterialCommunityIcons name="star-minus" size={20} color="#DC2626" />
                    <View style={styles.pointsInfo}>
                      <Text style={styles.pointsLabel}>Points Used</Text>
                      <Text style={[styles.pointsValue, styles.pointsUsedValue]}>-{pointsUsed} points</Text>
                    </View>
                  </View>
                )}
                {pointsEarned > 0 && (
                  <View style={styles.pointsRow}>
                    <MaterialCommunityIcons name="star-plus" size={20} color="#00A86B" />
                    <View style={styles.pointsInfo}>
                      <Text style={styles.pointsLabel}>Points Earned</Text>
                      <Text style={[styles.pointsValue, styles.pointsEarnedValue]}>+{pointsEarned} points</Text>
                    </View>
                  </View>
                )}
              </View>
        ...
        <View style={styles.modalActions}>
          <TouchableOpacity style={styles.closeModalButton} onPress={handleClose} activeOpacity={0.75}>
            <MaterialCommunityIcons name="close" size={18} color="#64748b" />
            <Text style={styles.closeModalText}>Close</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.downloadReceiptButton, isDownloading && styles.downloadReceiptButtonLoading]}
            onPress={() => onDownload(order._id, order.checkoutCode)}
            disabled={isDownloading}
            activeOpacity={0.8}
          >
...
const OrderHistoryScreen = ({ navigation }) => {
  ...
  const onRefresh = () => { setRefreshing(true); fetchOrderList(); };

  const handleDownloadReceipt = async (orderId, checkoutCode) => {
    setDownloadingId(orderId);
    try {
      const result = await downloadReceipt(orderId, checkoutCode);
      if (result.success) Alert.alert("Success", "Receipt downloaded successfully");
    } catch (err) {
  ...
  const getFilteredOrders = useCallback(() => {
    ...
    switch (selectedFilter) {
      case "week":     return orders.filter(o => new Date(o.confirmedAt) >= startOfWeek);
      case "month":    return orders.filter(o => new Date(o.confirmedAt) >= startOfMonth);
      case "discount": return orders.filter(o => (o.discountBreakdown?.total || 0) > 0 || o.seniorPwdDiscountAmount > 0);
  ...
  const formatPrice = (n) => `₱${(n || 0).toFixed(2)}`;
  const stats = {
    totalSpent:     formatPrice(filteredOrders.reduce((s, o) => s + (o.finalAmountPaid || 0), 0)),
    totalPoints:    filteredOrders.reduce((s, o) => s + (o.loyaltyDiscount?.pointsEarned || o.pointsEarned || 0), 0),
    avgTransaction: filteredOrders.length > 0
      ? formatPrice(filteredOrders.reduce((s, o) => s + (o.finalAmountPaid || 0), 0) / filteredOrders.length)
      : "₱0.00",
  };

  const openDetails  = useCallback((order) => { setSelectedOrder(order); setShowDetails(true); }, []);
      ...
      >
        ...
        <View style={styles.statsGrid}>
          {[
            ...
            { value: stats.totalPoints.toLocaleString(), label: "Points Earned" },
        ...
        {filteredOrders.length > 0 ? (
          filteredOrders.map((order, i) => (
            <OrderCard
              key={order._id || order.checkoutCode}
              order={order}
              index={i}
        ...
        <View style={styles.tipsCard}>
          ...
          <View style={{ flex: 1, marginLeft: 16 }}>
            ...
            <Text style={styles.tipsText}>Discounts and points are automatically calculated</Text>
          </View>
        </View>
      </ScrollView>

      {/* Modal — keep mounted while selectedOrder exists so exit animation can play */}
      {selectedOrder && (
        <OrderDetailsModal
          order={selectedOrder}
          visible={showDetails}
...
const styles = StyleSheet.create({
  ...
  statsGrid: { flexDirection: "row", paddingHorizontal: 24, marginTop: 10, marginBottom: 25 },
  statBox:   { flex: 1, backgroundColor: "#fff", marginHorizontal: 4, padding: 16, borderRadius: 20, alignItems: "center", borderWidth: 1, borderColor: "#f1f5f9" },
  ...
  filterChip:       { marginRight: 10, borderRadius: 20, borderWidth: 1, overflow: "hidden" },
  filterChipInner:  { paddingHorizontal: 18, paddingVertical: 10 },
  filterChipText:   { fontSize: 13, fontWeight: "700" },

  sectionHeader: { flexDirection: "row", justifyContent: "space-between", paddingHorizontal: 24, marginBottom: 16, alignItems: "center" },
  sectionTitle:  { fontSize: 18, fontWeight: "800", color: "#0f172a" },
  orderCount:    { fontSize: 13, color: "#64748b", fontWeight: "600" },

  orderCard:   { backgroundColor: "#fff", marginHorizontal: 24, padding: 20, borderRadius: 24, marginBottom: 12, borderWidth: 1, borderColor: "#f1f5f9" },
  orderHeader: { flexDirection: "row", alignItems: "center", marginBottom: 16 },
  storeBadge:  { width: 36, height: 36, borderRadius: 12, backgroundColor: "#f8fafc", justifyContent: "center", alignItems: "center" },
  ...
  pointsBadge: { fontSize: 11, color: "#00A86B", fontWeight: "800", marginTop: 4, backgroundColor: "rgba(0,168,107,0.1)", paddingHorizontal: 8, paddingVertical: 3, borderRadius: 10, overflow: "hidden", textAlign: "center" },

  itemsSection: { marginBottom: 16, paddingBottom: 16, borderBottomWidth: 1, borderBottomColor: "#f1f5f9" },
  itemsHeader:  { flexDirection: "row", alignItems: "center", marginBottom: 12, gap: 8 },
  itemsLabel:   { fontSize: 13, fontWeight: "700", color: "#64748b" },
  itemsList:    { gap: 8 },
  itemRow:      { flexDirection: "row", alignItems: "center", gap: 8 },
  itemName:     { flex: 1, fontSize: 14, color: "#334155" },
  itemQuantity: { fontSize: 12, color: "#94a3b8", fontWeight: "600", minWidth: 40 },
  itemPrice:    { fontSize: 13, fontWeight: "700", color: "#0f172a" },
  moreItems:    { fontSize: 12, color: "#94a3b8", fontStyle: "italic", marginTop: 4 },

  orderFooter:      { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 12 },
  chip:             { flexDirection: "row", alignItems: "center", backgroundColor: "#f8fafc", paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12 },
  statusChip:       { backgroundColor: "rgba(0,168,107,0.1)" },
  chipText:         { fontSize: 11, fontWeight: "700", color: "#64748b", marginLeft: 6 },
  ...
  detailsText:      { fontSize: 13, fontWeight: "700", color: "#00A86B", marginRight: 2 },

  downloadButton:       { flexDirection: "row", alignItems: "center", justifyContent: "center", marginTop: 12, paddingVertical: 10, backgroundColor: "#F8FAFC", borderRadius: 12, gap: 8, borderWidth: 1, borderColor: "#E2E8F0" },
  downloadButtonActive: { backgroundColor: "#F0FDF8", borderColor: "#00A86B" },
  downloadButtonText:   { fontSize: 13, fontWeight: "600", color: "#00A86B", marginLeft: 4 },

  emptyContainer: { alignItems: "center", paddingVertical: 60, marginHorizontal: 24 },
  ...
  shopButton:     { paddingHorizontal: 32, paddingVertical: 12, backgroundColor: "#00A86B", borderRadius: 12 },
  shopButtonText: { fontSize: 14, fontWeight: "700", color: "#fff" },

  tipsCard:  { flexDirection: "row", alignItems: "center", backgroundColor: "#fff", marginHorizontal: 24, marginTop: 20, padding: 20, borderRadius: 24, borderWidth: 1, borderColor: "#f1f5f9" },
  tipsTitle: { fontSize: 14, fontWeight: "800", color: "#0f172a", marginBottom: 2 },
  ...
  modalContainer: { backgroundColor: "#fff", borderTopLeftRadius: 28, borderTopRightRadius: 28, maxHeight: "90%", paddingBottom: 8 },
  dragHandle:     { width: 40, height: 4, borderRadius: 2, backgroundColor: "#e2e8f0", alignSelf: "center", marginTop: 12, marginBottom: 4 },
  modalHeader:    { flexDirection: "row", justifyContent: "space-between", alignItems: "center", padding: 20, borderBottomWidth: 1, borderBottomColor: "#f1f5f9" },
  modalTitle:     { fontSize: 18, fontWeight: "800", color: "#0f172a" },
  modalSubtitle:  { fontSize: 14, color: "#94a3b8", marginTop: 2 },
  closeButton:    { width: 40, height: 40, borderRadius: 12, backgroundColor: "#f8fafc", justifyContent: "center", alignItems: "center" },
  modalContent:   { padding: 20 },
  modalSection:   { marginBottom: 24 },

  infoCard:   { backgroundColor: "#f8fafc", borderRadius: 16, padding: 16, gap: 16 },
  infoRow:    { flexDirection: "row", alignItems: "center", gap: 12 },
  infoText:   { flex: 1 },
  infoLabel:  { fontSize: 12, color: "#64748b", fontWeight: "700", marginBottom: 2 },
  infoValue:  { fontSize: 14, fontWeight: "600", color: "#0f172a" },
  statusValue:{ color: "#00A86B" },

  modalItemsList:     { backgroundColor: "#f8fafc", borderRadius: 16, padding: 16, gap: 12 },
  modalItemCard:      { flexDirection: "row", alignItems: "center", paddingVertical: 4 },
  modalItemIcon:      { width: 32, alignItems: "center" },
  modalItemInfo:      { flex: 1, marginLeft: 8 },
  modalItemName:      { fontSize: 14, fontWeight: "600", color: "#0f172a", marginBottom: 4 },
  ...
  pricingCard:        { backgroundColor: "#f8fafc", borderRadius: 16, padding: 16, gap: 12 },
  ...
  totalDiscountRow:   { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingTop: 8, marginTop: 4, borderTopWidth: 1, borderTopColor: "#e2e8f0" },
  totalDiscountLabel: { fontSize: 14, fontWeight: "600", color: "#0f172a" },
  totalDiscountValue: { fontSize: 14, fontWeight: "700", color: "#059669" },
  totalRow:           { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: 8, paddingTop: 12, borderTopWidth: 1, borderTopColor: "#e2e8f0" },
  totalLabel:         { fontSize: 16, fontWeight: "700", color: "#0f172a" },
  totalValue:         { fontSize: 18, fontWeight: "800", color: "#0f172a" },

  capsCard: { backgroundColor: "#f8fafc", borderRadius: 16, padding: 16, gap: 8 },
  ...
  capNote:  { fontSize: 12, color: "#94a3b8", fontStyle: "italic", marginTop: 4 },

  pointsCard:        { backgroundColor: "#f8fafc", borderRadius: 16, padding: 16, gap: 16 },
  pointsRow:         { flexDirection: "row", alignItems: "center", gap: 6 },
  pointsInfo:        { flex: 1 },
  pointsLabel:       { fontSize: 12, color: "#64748b", fontWeight: "700", marginBottom: 2 },
  pointsValue:       { fontSize: 14, fontWeight: "600" },
  pointsUsedValue:   { color: "#DC2626" },
  pointsEarnedValue: { color: "#00A86B" },

  modalActions:                 { flexDirection: "row", padding: 20, gap: 12, borderTopWidth: 1, borderTopColor: "#f1f5f9" },
  closeModalButton:             { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", paddingVertical: 14, borderWidth: 1, borderColor: "#e2e8f0", borderRadius: 12, gap: 8 },
  closeModalText:               { fontSize: 14, fontWeight: "700", color: "#64748b" },
  downloadReceiptButton:        { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", paddingVertical: 14, backgroundColor: "#00A86B", borderRadius: 12, gap: 8 },
  downloadReceiptButtonLoading: { backgroundColor: "#059669" },
  downloadReceiptButtonText:    { fontSize: 14, fontWeight: "700", color: "#fff" },
});
