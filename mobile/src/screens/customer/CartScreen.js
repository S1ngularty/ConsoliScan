...
const CartScreen = ({ navigation }) => {
  ...
  const availableVouchers = [
    { code: 'WELCOME10', discount: 10, type: 'percentage', minSpend: 100, description: '10% off on first purchase' },
    { code: 'SAVE50', discount: 50, type: 'fixed', minSpend: 200, description: 'Save ₱50 on orders above ₱200' },
    { code: 'SCAN20', discount: 20, type: 'percentage', minSpend: 150, description: '20% off for scanning products' },
  ];
    ...
    Alert.alert(
      ...
      [
        ...
        { 
          text: 'Remove', 
          style: 'destructive',
          onPress: () => dispatch(removeToCart(itemId))
        },
      ]
    );
  };
  ...
  const handleApplyVoucher = () => {
    ...
    if (totalPrice < voucher.minSpend) {
      Alert.alert('Minimum Spend Required', `Minimum spend of ₱${voucher.minSpend} required for this voucher`);
      return;
    }
    ...
    Alert.alert('Success', `Voucher applied! You saved ₱${calculatedDiscount.toFixed(2)}`);
  };
  ...
  const renderCartItem = (item) => (
    <View key={item._id} style={styles.cartItem}>
      <Image
        source={{ uri: item.images?.[0]?.url || item.product?.images?.[0]?.url || 'https://via.placeholder.com/100' }}
            ...
            {appliedVoucher ? (
              <View style={styles.appliedVoucher}>
                <View style={styles.voucherInfo}>
                  <MaterialCommunityIcons name="ticket-percent" size={20} color="#4CAF50" />
                  <View style={styles.voucherDetails}>
                    <Text style={styles.voucherCode}>{appliedVoucher.code}</Text>
                    <Text style={styles.voucherDescription}>{appliedVoucher.description}</Text>
                  </View>
                </View>
                <TouchableOpacity 
              ...
              {availableVouchers.map((voucher) => (
                <View key={voucher.code} style={styles.voucherItem}>
                  <View style={styles.voucherItemIcon}>
                    <MaterialCommunityIcons name="ticket-percent" size={18} color="#FF9800" />
                  </View>
                  <View style={styles.voucherItemContent}>
                    <Text style={styles.voucherItemCode}>{voucher.code}</Text>
                    <Text style={styles.voucherItemDesc}>{voucher.description}</Text>
                  ...
                  >
                    <Text style={[
                      ...
                      totalPrice < voucher.minSpend && styles.useVoucherTextDisabled
                    ]}>
                      Use
                    </Text>
                  </TouchableOpacity>
        ...
        {cart.length > 0 && (
          <View style={styles.pointsCard}>
            ...
            <Text style={styles.pointsText}>
              You'll earn {Math.floor(calculateFinalTotal() / 10)} loyalty points with this purchase
            </Text>
          </View>
        )}
      </ScrollView>
