...
import type { IQueue, IQueueMethods, IQueueVirtuals } from "./queue.types.js";

type QueueModel = Model<IQueue, {}, IQueueMethods, IQueueVirtuals>;
...
>(
  {
    ...
    status: {
      type: String,
      enum: ["PENDING", "SCANNED", "LOCKED", "PAID", "CANCELLED", "EXPIRED"],
      default: "PENDING",
    },
    scannedAt: Date,
    lockedAt: Date,
...
queueSchema.virtual("totalItems").get(function () {
  return this.items?.reduce((sum, item) => sum + item.quantity, 0) || 0;
});

// Method to check if queue is expired
// this document object method is disabled since the qr will not be expired anymore
// checkoutQueueSchema.methods.isExpired = function () {
//   return new Date() > this.expiresAt;
// };

// Method to check if BNPC discount was applied
queueSchema.methods.hasBNPCDiscount = function () {
  return (this.totals?.bnpcDiscount ?? 0) > 0;
};

// Method to get discount summary
queueSchema.methods.getDiscountSummary = function () {
  return {
...
export type QueueDocument = HydratedDocument<
  IQueue,
  IQueueMethods,
  {},
  IQueueVirtuals
...
export const Queue = model("CheckoutQueue", queueSchema);
